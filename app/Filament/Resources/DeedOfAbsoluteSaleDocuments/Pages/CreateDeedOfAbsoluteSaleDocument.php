...
class CreateDeedOfAbsoluteSaleDocument extends CreateRecord
{
  ...
  public function mount(): void
  {
    ...
    if ($templateId) {
      $this->form->fill([
        'deed_of_absolute_sale_template_id' => $templateId,
      ]);
    }
  }
              ...
              ->options(
                DeedOfAbsoluteSaleTemplate::query()
                  ...
                  ->mapWithKeys(fn ($template) => [
                    $template->id => sprintf(
                      'Template #%d — used in %d %s',
                      $template->id,
                      $template->documents?->count(),
                      str('document')->plural($template->documents?->count())
                    ),
                  ])
              )
              ->searchable()
